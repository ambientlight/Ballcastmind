...
class ConvSimpleWithSingleOnTopModel(ModelDescriptor):
    ...
    def create_model(self, space: Optional[Dict[str, Any]] = None) -> Model:
        ...
        x = layers.MaxPooling2D(2)(x)
        x = layers.Flatten()(x)
        pos_x_dense = layers.Dense(64, activation='relu')(x)
        pos_y_dense = layers.Dense(64, activation='relu')(x)
        pos_z_dense = layers.Dense(64, activation='relu')(x)
        rot_x_dense = layers.Dense(64, activation='relu')(x)
        rot_y_dense = layers.Dense(64, activation='relu')(x)
        rot_z_dense = layers.Dense(64, activation='relu')(x)
